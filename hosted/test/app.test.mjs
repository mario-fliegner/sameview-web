// Regression tests for the request-handling behavior in hosted/app.js,
// mirroring test/app.test.mjs (the root application's equivalent). Hosted's
// app.js duplicates the root's proven request-handling pattern deliberately
// (see hosted/app.js's own header comment) rather than sharing code across
// the two independently deployed applications, so this test suite is
// duplicated the same way, adapted only for the Hosted path.
//
// hosted/app.js is a CommonJS module (Passenger's Node loader requires
// that — see docs/deployment.md); Node's ESM loader can still import its
// named exports.
//
// hosted/app.js's own startup path (the real `.listen()`) runs
// unconditionally unless `NODE_ENV === "test"` — see docs/deployment.md for
// why it must NOT be gated behind `require.main === module` instead (that
// silently breaks under Passenger). NODE_ENV is set here, before app.js is
// loaded, so this file only exercises the pure request-handling helpers
// below, without starting a real server on a real port. A dynamic import()
// is required for that ordering to actually work: a static import at the
// top of this file would be hoisted and evaluated before the
// `process.env.NODE_ENV` assignment below ever runs. See
// hosted/test/passenger-boot.test.mjs for the separate test that verifies
// the real startup path actually listens.
