...
- **Implement:** Decode and validate actual image content, enforce the 40-megapixel input limit, remove embedded metadata through decode/re-encode, and optimize the two comparison images for embedding; the already-normalized branding asset from Phase 6, when present, is passed through unchanged — no further decode, scaling, optimization or re-encoding, since it is already the final asset — and report failure without altering workspace or previous outcomes.
