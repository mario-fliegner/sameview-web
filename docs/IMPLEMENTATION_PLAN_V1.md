...
These decisions need narrow technical or product clarification in their named iteration; they do not justify advance infrastructure. All three WordPress-track rows above are now resolved — repository location, artifact format and host-isolation mechanism were each implementation-time technical decisions, not open product questions; `EMBED_IN_WEBSITE.md` and `WORDPRESS_INTEGRATION.md` already settle every product-level requirement they must satisfy.
