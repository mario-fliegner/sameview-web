...
These decisions need narrow technical or product clarification in their named iteration; they do not justify advance infrastructure. Of the WordPress-track rows above, repository location is now resolved; the remaining two (artifact format, host-isolation mechanism) are additionally implementation-time technical decisions, not open product questions — `EMBED_IN_WEBSITE.md` and `WORDPRESS_INTEGRATION.md` already settle every product-level requirement they must satisfy.
